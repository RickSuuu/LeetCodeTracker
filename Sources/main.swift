...
class TrackerStore: ObservableObject {
    ...
    init() {
        categories = Self.defaultData()
        loadProgress()
    }

    func toggle(_ problemId: Int) {
        for ci in categories.indices {
            for pi in categories[ci].problems.indices {
                if categories[ci].problems[pi].id == problemId {
                    categories[ci].problems[pi].done.toggle()
                    saveProgress()
                    return
                }
            }
    ...
    var totalDone: Int { categories.flatMap(\.problems).filter(\.done).count }
    var totalCount: Int { categories.flatMap(\.problems).count }

    func resetAll() {
        for ci in categories.indices {
            for pi in categories[ci].problems.indices {
                categories[ci].problems[pi].done = false
            }
        }
        saveProgress()
    }
    ...
    private func saveProgress() {
        let doneIds = Set(categories.flatMap(\.problems).filter(\.done).map(\.id))
    ...
    private func loadProgress() {
        ...
        for ci in categories.indices {
            for pi in categories[ci].problems.indices {
                if doneSet.contains(categories[ci].problems[pi].id) {
                    categories[ci].problems[pi].done = true
                }
            }
        }
    }
